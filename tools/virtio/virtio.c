...
static int consume_pending_requests(void) {
    ...
    __atomic_store_n(p_need_wakeup, 0, memory_order_release);
    ...
    while (true) {
        ...
        if (req_front != req_rear) {
             ...
             * eventfd.
             */
            __atomic_store_n(p_need_wakeup, 1, memory_order_release);
             ...
             * setting the flag.
             */
            req_rear = __atomic_load_n(p_req_rear, memory_order_acquire);
            if (req_front == req_rear) {
...
void handle_virtio_requests(void) {
    ...
    __atomic_store_n(&virtio_bridge->need_wakeup, 1, memory_order_release);
