...
static int handle_power_protocol_attributes(SCMIDev *dev, uint16_t token,
    ...
    scmi_make_response(ctx, SCMI_PROTO_ID_POWER,
...
static int handle_power_domain_attributes(SCMIDev *dev, uint16_t token,
    ...
    scmi_make_response(ctx, SCMI_PROTO_ID_POWER,
                       SCMI_POWER_MSG_POWER_DOMAIN_ATTRIBUTES, token,
                       SCMI_SUCCESS);
    struct scmi_msg_resp_power_domain_attributes *attr = scmi_resp_write(
        ctx, sizeof(struct scmi_msg_resp_power_domain_attributes));
    attr->flags = (1U << 29);
...
static int handle_power_state_set(SCMIDev *dev, uint16_t token,
    ...
    if (req_iov->iov_len < sizeof(struct scmi_request) +
                               sizeof(struct scmi_msg_req_power_state_set))
        return SCMI_ERR_PARAMS;
    struct scmi_request *req = req_iov->iov_base;
...
static int handle_power_state_get(SCMIDev *dev, uint16_t token,
    ...
    if (ctx->capacity < sizeof(struct scmi_response) + sizeof(uint32_t)) {
        return scmi_make_response(ctx, SCMI_PROTO_ID_POWER,
                                  SCMI_POWER_MSG_POWER_STATE_GET, token,
                                  SCMI_ERR_RANGE);
    }
...
int virtio_scmi_power_handle_req(SCMIDev *dev, uint8_t msg_id, uint16_t token,
    ...
    case SCMI_COMMON_MSG_VERSION:
        return handle_power_version(dev, token, req_iov, ctx);
    case SCMI_COMMON_MSG_PROTOCOL_ATTRIBUTES:
        return handle_power_protocol_attributes(dev, token, req_iov, ctx);
    case SCMI_COMMON_MSG_MESSAGE_ATTRIBUTES:
        return scmi_make_response(ctx, SCMI_PROTO_ID_POWER, msg_id, token,
                                  SCMI_ERR_SUPPORT);
    case SCMI_POWER_MSG_POWER_DOMAIN_ATTRIBUTES:
        return handle_power_domain_attributes(dev, token, req_iov, ctx);
    case SCMI_POWER_MSG_POWER_STATE_SET:
        return handle_power_state_set(dev, token, req_iov, ctx);
    case SCMI_POWER_MSG_POWER_STATE_GET:
        return handle_power_state_get(dev, token, req_iov, ctx);
    case SCMI_POWER_MSG_POWER_STATE_NOTIFY:
        return handle_power_state_notify(dev, token, req_iov, ctx);
    default:
