...
static void virtio_console_rx_completion_handler(void *param, int res) {
    ...
    update_used_ring(vq, req->req.idx, res);
...
static void virtio_console_pty_rx_handler(int fd, int epoll_type, void *param) {
    ...
    if (dev->pending_rx_req) {
        ...
        io_uring_prep_readv(sqe, dev->master_fd, req->req.iov, req->req.iovcnt,
                        0);
    ...
    if (virtqueue_is_empty(vq)) {
        disable_event_poll(&dev->rx_poll_event);
        virtio_inject_irq(vq);
        return;
    }
    ...
    req->req.io_completion_event.handler = virtio_console_rx_completion_handler;
    ...
    io_uring_prep_readv(sqe, dev->master_fd, req->req.iov, req->req.iovcnt, 0);
    io_uring_sqe_set_data(sqe, &req->req.io_completion_event);
    // io_uring_submit(ring);
}
...
int virtio_console_init(VirtIODevice *vdev) {
    ...
    dev->rx_poll_event = (struct poll_event){
        ...
        .fd = dev->master_fd,
        .epoll_type = POLLIN,
        .handler = virtio_console_pty_rx_handler,
        .param = vdev,
        .active = false,
    };
...
static void virtio_console_txq_process(VirtIODevice *vdev, VirtQueue *vq) {
    ...
    while (!virtqueue_is_empty(vq) && quota > 0) {
        ...
        while (!virtqueue_is_empty(vq) && quota > 0) {
            if (virtio_console_handle_one_tx_req(vdev->dev, vq) == 0) {
                submitted++;
            }
            quota--;
        }
...
static void virtio_console_tx_completion_handler(void *param, int res) {
    ...
    update_used_ring(vq, req->req.idx, 0);
    free(req->req.iov);
    free(req);
}
...
/// @param vq Pointer to the VirtQueue structure.
/// @return 0 on success, -1 on failure.
static int virtio_console_handle_one_tx_req(ConsoleDev *dev, VirtQueue *vq) {
    ...
    req->req.vq = vq;
    req->req.io_completion_event.base.type = EVENT_TYPE_IO_COMPLETION;
    req->req.io_completion_event.handler = virtio_console_tx_completion_handler;
    req->req.io_completion_event.param = req;
    req->req.io_completion_event.free_on_completion = false;
    ...
    io_uring_prep_writev(sqe, dev->master_fd, req->req.iov, req->req.iovcnt, 0);
    io_uring_sqe_set_data(sqe, &req->req.io_completion_event);
